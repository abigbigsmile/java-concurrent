...
import com.smile.thread.common.MessageResponse;
...
public class FTPUtil {
    ...
    public MessageResponse downloadFile(String localPath, String remotePath, String fileName) throws IOException {
        ...
        if(connectServer()){
            try {
                ...
                if(!hasPath){
                    return MessageResponse.errorMessage("目标路径不存在！");
                }
                int reply = ftpClient.getReplyCode();
                if (!FTPReply.isPositiveCompletion(reply)) {
                    ftpClient.disconnect();
                    return MessageResponse.errorMessage("ftp服务器不响应！");
                }
                ftpClient.enterLocalPassiveMode();
                ftpClient.setControlEncoding("UTF-8");
                ...
                e.printStackTrace();
                return MessageResponse.errorMessage("下载文件失败！");
            }finally {
                if(fos != null)fos.close();
                ftpClient.disconnect();
            }
        }
        return MessageResponse.successMessage("下载文件成功");
...
//        if(isUpload) System.out.println("上传ftp成功 ！");
        MessageResponse messageResponse = ftpUtil.downloadFile("C:\\Users\\Smile\\Desktop\\image", "image", "2.jpg");
        System.out.println();
    }

...
![Alt text](./1558778999583.png)
![Alt text](./1558779009921.png)
...
![Alt text](./1558779136894.png)
![Alt text](./1558779155498.png)


前面的可重入锁用一个int值（state）来表示锁的状态，数值可以表示同一线程重入的次数，那对于读写锁，有几个状态需要解决：
![Alt text](./1558779145541.png)

接下来，我们来读一下源码：
#####获取写锁（排它锁）：
![Alt text](./1558779176657.png)
...
当c==0，设拿到锁的线程为当前线程，并设重入数为1，firstReader = current;firstReaderHoldCount = 1;若firstReader == current，firstReaderHoldCount++;其他情况表示其他线程获取读锁，则使用HoldCounter计重入数
 ![Alt text](./1558779218100.png)

而HoldCounter是用ThreadLocal进行存储，保证安全性
![Alt text](./1558779226974.png)
...
而ThreadLocalMap是Thread的一个属性，在getMap(t)中就返回Thread的ThreadLocalMap
 ![Alt text](./1558779403711.png)

set():
 ![Alt text](./1558779407584.png)

setInitialValue():
 ![Alt text](./1558779412583.png)
